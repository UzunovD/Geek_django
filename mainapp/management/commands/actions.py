...
from django.db.models import Q, F, When, IntegerField, DecimalField
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for orderitem in test_orderss:
            print(f'{orderitem.action_order:2}: order №{orderitem.pk:3}:\
                   {orderitem.product.name:25}: discount\
                   ${abs(orderitem.total_discount):6.2f} | \
